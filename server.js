...
io.on("connection", (socket) => {
  socket.on("user", (user) => {
    ...
    socket.on("typing", (whosit) => {
      socket.broadcast.to(user.room).emit("whosit", whosit);
    })

    socket.on("chat message", (msg) => {
      if (msg.message == "#Burn") {
        Msg.deleteMany({ deleted:true })
          .then(function () {
    ...
    socket.on("todrop", (r2d) => {
      ...
      Msg.updateMany({ room: r2d }, { deleted: true })
        .then(function () {
          console.log("Data deleted");
        })
        .catch(function (error) {
  ...
  socket.on("disconnect", () => {
    let index = usersOnline.findIndex((x) => x.sid === socket.id);
